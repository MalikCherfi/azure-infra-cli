...
```bash
export OWNER="prenom-nom"            # votre prénom-nom (minuscules, sans espaces)
...
Voici le bloc à ajouter dans votre `provision.sh` après la création des ressources de calcul :

```bash
# ── Réseau (VNet + NSG) ───────────────────────────────────────────────────────
echo ""
echo "▶ [8/8] Réseau (VNet + subnets + NSG)..."
...
echo "✅ VNet créé : $VNET_NAME (frontend: 10.0.1.0/24 / backend: 10.0.2.0/24)"
...
echo "✅ NSG créé et associé à subnet-frontend (HTTP:100 / HTTPS:110 / Deny:4000)"
```

Et dans `destroy.sh`, ajouter **avant** la suppression des resources de calcul (le NSG doit être désassocié avant suppression) :

```bash
# ── Réseau ────────────────────────────────────────────────────────────────────
# Désassocier le NSG du subnet avant suppression (obligatoire)
...
# Voir les adresses IP utilisées dans un subnet
az network vnet subnet show \
  --name "subnet-frontend" --vnet-name "$VNET_NAME" \
  --resource-group "$RG" \
...
# Ajouter une règle SSH (port 22) — à utiliser uniquement en dev depuis votre IP !
...
az network nsg rule create \
  ...
  --description "SSH depuis mon IP uniquement — à supprimer en production"
```
